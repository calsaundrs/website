/**
 * Test Script: Google Place IDs for Multiple Venues
 * 
 * This script tests the Google Place ID creation process on multiple venues
 * before running it on all venues.
 * 
 * Usage: node test-multiple-venues-place-id.js
 */
...
async function main() {
  try {
    ...
    const results = await tester.processTestVenues();
    ...
    if (results.failed === 0) {
      ...
      console.log('🔧 Please check the errors before running the full script.');
      process.exit(1);
    }
  } catch (error) {
...
module.exports = { MultipleVenueTester }; 