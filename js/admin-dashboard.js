document.addEventListener('DOMContentLoaded', () => {
    ...
    window.mergeDuplicateVenues = async function() {
        try {
            console.log('Admin Dashboard: Starting automatic duplicate venue merge...');
            ...
            if (response.ok) {
                const result = await response.json();
                console.log('Admin Dashboard: Merge results:', result);
                ...
                const errorData = await response.text();
                console.error('Admin Dashboard: Merge failed:', response.status, errorData);
                alert(`Merge failed: ${response.status} - ${errorData}`);
            }
        } catch (error) {
            console.error('Admin Dashboard: Error during merge:', error);
