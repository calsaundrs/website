...
if (!admin.apps.length) {
    try {
        ...
        if (missingVars.length > 0) {
            ...
            throw new Error('Firebase environment variables required');
        }
...
// Same venue processing logic as the API
...
async function generateAllVenuePages() {
    console.log('🚀 Starting Database-based Venue SSG Build...');
    ...
    try {
        ...
        console.log('📡 Fetching venues from Firebase...');
        ...
        console.log(`📄 Found ${venues.length} venues with Cloudinary images to generate`);
...
module.exports = { generateAllVenuePages };

// Run if called directly
if (require.main === module) {
    generateAllVenuePages().then(result => {
        if (result.success) {
            console.log('🎉 Database-based SSG completed successfully!');
