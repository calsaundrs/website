...
exports.handler = async function (event, context) {
    console.log('Fix event status fields function called');
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            // Check if event has uppercase Status but no lowercase status
            if (upperStatus && !data.status) {
                eventsToUpdate.push({
                    id: doc.id,
                    name: data.name || data['Event Name'] || 'Untitled Event',
                    currentStatus: currentStatus,
                    upperStatus: upperStatus,
                    newStatus: upperStatus.toLowerCase()
                });
            }
        });
        ...
        // Update events with missing lowercase status
        ...
        return {
            ...
            body: JSON.stringify({
                success: true,
                message: 'Event status fields fixed successfully',
        ...
        console.error('Fix event status fields failed:', error);
        return {
            statusCode: 500,
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                error: 'Fix event status fields failed',
