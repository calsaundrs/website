...
exports.handler = async (event, context) => {
  try {
    console.log('Starting event image migration from Airtable to Firestore...');
    ...
    // Get all events from Airtable with image data
    ...
    console.log(`Found ${airtableRecords.length} events in Airtable`);
    ...
    firestoreSnapshot.forEach(doc => {
      const data = doc.data();
      const key = data.slug || data.name; // Use slug or name as key
      firestoreEvents[key] = {
        id: doc.id,
        data: data
      };
    ...
    console.log(`Found ${Object.keys(firestoreEvents).length} events in Firestore`);
    ...
    for (const record of airtableRecords) {
      try {
        ...
        if (cloudinaryPublicId) {
          ...
          console.log(`Adding Cloudinary Public ID for ${eventName}: ${cloudinaryPublicId}`);
        }

        if (promoImage && promoImage.length > 0) {
          imageUpdates['Promo Image'] = promoImage;
          console.log(`Adding Promo Image for ${eventName}: ${promoImage[0].url}`);
        }
    ...
    return {
      ...
      body: JSON.stringify({ 
        success: true,
        message: 'Event image migration completed',
