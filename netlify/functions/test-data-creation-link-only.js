...
exports.handler = async function (event, context) {
    console.log('Creating link-only test data...');
    ...
    try {
        ...
        // Create link-only test event in Airtable (adding only Link, no Category)
        try {
            console.log('Creating link-only test event in Airtable...');
            const testEventData = {
                'Event Name': 'Link-Only Test Event',
                'Description': 'This is a link-only test event with Link but no Category',
            ...
            console.log('Link-only event data to create:', testEventData);
            ...
            console.log('Creating link-only test event in Firestore...');
            ...
            console.error('Error creating link-only test event:', error);
            results.events.error = error.message;
        }
        
        // Create link-only test venue in Airtable (same as minimal)
        try {
            console.log('Creating link-only test venue in Airtable...');
            const testVenueData = {
                'Name': 'Link-Only Test Venue',
                'Description': 'This is a link-only test venue',
                'Address': '123 Link Street, Birmingham',
                'Status': 'Approved',
                'Contact Email': 'test@brumoutloud.co.uk'
            };
            
            console.log('Link-only venue data to create:', testVenueData);
            ...
            console.log('Creating link-only test venue in Firestore...');
            ...
            console.error('Error creating link-only test venue:', error);
        ...
        return {
            ...
            body: JSON.stringify({
                success: true,
                message: 'Link-only test data created successfully!',
                results: results
            })
        };
        
    } catch (error) {
        console.error('Link-only test data creation failed:', error);
        return {
            statusCode: 500,
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                error: 'Link-only test data creation failed',
