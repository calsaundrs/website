...
<body>
    ...
    <script>
            ...
            try {
                ...
                if (data.success && data.events.length > 0) {
                    ...
                    if (recurringEvents.length > 0) {
                        eventsHTML += '<section class="mb-12"><div class="bg-gray-800/50 backdrop-blur-sm border border-gray-700 p-8 rounded-2xl"><h3 class="font-anton text-3xl text-white mb-6 flex items-center"><div class="w-12 h-12 bg-gradient-to-br from-green-500/20 to-emerald-600/20 rounded-full flex items-center justify-center mr-4"><i class="fas fa-redo text-green-400 text-xl"></i></div>Regular Events<span class="ml-4 bg-green-500/20 text-green-300 text-sm px-4 py-2 rounded-full font-semibold">' + recurringEvents.length + ' event' + (recurringEvents.length !== 1 ? 's' : '') + '</span></h3><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">' + recurringEvents.map(event => renderEventCard(event, true)).join('') + '</div></div></section>';
                    }
                    
                    // One-off Events Section
                    if (oneOffEvents.length > 0) {
                        eventsHTML += '<section class="mb-12"><div class="bg-gray-800/50 backdrop-blur-sm border border-gray-700 p-8 rounded-2xl"><h3 class="font-anton text-3xl text-white mb-6 flex items-center"><div class="w-12 h-12 bg-gradient-to-br from-blue-500/20 to-indigo-600/20 rounded-full flex items-center justify-center mr-4"><i class="fas fa-calendar-day text-blue-400 text-xl"></i></div>Upcoming Events<span class="ml-4 bg-blue-500/20 text-blue-300 text-sm px-4 py-2 rounded-full font-semibold">' + oneOffEvents.length + ' event' + (oneOffEvents.length !== 1 ? 's' : '') + '</span></h3><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">' + oneOffEvents.map(event => renderEventCard(event, false)).join('') + '</div></div></section>';
                    }
        ...
        function renderEventCard(event, isRecurring) {
            ...
            return '<div class="venue-card rounded-xl overflow-hidden relative">' + 
