...
<body>
    ...
    <!-- The Architectural Poster Layout -->
    <main class="w-full bg-[#050505] min-h-screen">
        
        <!-- Top Poster Section: Edge-to-edge, precise, no borders -->
        <div class="w-full relative aspect-square md:aspect-[21/9] lg:aspect-[2.5/1] bg-black overflow-hidden flex items-center justify-center">
            {{#if event.image}}
            <!-- Clean, unfiltered image source filling the container precisely -->
            <img src="{{event.image.url}}" alt="Event Poster" class="absolute w-full h-full object-cover opacity-90 object-center">
            {{else}}
            <div class="w-full h-full flex items-center justify-center bg-[#111]">
                <span class="text-white/40 tracking-widest text-sm">NO IMAGE AVAILABLE</span>
            </div>
            {{/if}}
            <!-- Optional subtle gradient overlay at bottom to ensure breadcrumb visibility if image is bright -->
            <div class="absolute bottom-0 w-full h-32 bg-gradient-to-t from-[#0A0A0A] to-transparent z-10"></div>
        </div>

        <!-- Rigorous Content Grid -->
        <div class="max-w-[1400px] mx-auto w-full flex flex-col relative z-20 -mt-16 md:-mt-20">
            
            <!-- Breadcrumbs / Top alignment -->
            <nav class="w-full px-6 py-6 flex items-center justify-between text-xs font-medium uppercase tracking-widest text-white/50 border-b border-white/10 bg-[#050505]">
                <a href="/events" class="hover:text-white transition-colors">BACK TO EVENTS</a>
                <span class="text-[var(--color-toxic)]">{{event.category.[0]}}</span>
            </nav>

            <!-- Massive Title Block -->
            <div class="w-full px-6 py-12 md:py-24 border-b border-white/10 bg-[#050505]">
                <h1 class="text-white font-black text-6xl md:text-8xl lg:text-9xl uppercase leading-[0.85] tracking-tighter m-0 font-display break-words">
                    {{event.name}}
                </h1>
                
                <!-- Auxiliary Tags -->
                <div class="flex flex-wrap gap-2 mt-8 opacity-80">
                    {{#each event.category}}
                    <span class="text-white border border-white/20 px-3 py-1 font-bold text-xs uppercase tracking-[0.2em]">{{this}}</span>
                    {{/each}}
            ...
            <div class="w-full grid grid-cols-1 md:grid-cols-3 border-b border-white/10 bg-[#050505]">
                <!-- Date -->
                <div class="px-6 py-8 md:py-12 border-b md:border-b-0 md:border-r border-white/10 flex flex-col">
                    <span class="text-white/40 text-xs font-bold uppercase tracking-[0.2em] mb-3 font-display">DATE</span>
                    <span class="text-white text-2xl font-black uppercase tracking-tight">{{formatDateOnly event.date}}</span>
                </div>
                <!-- Time -->
                <div class="px-6 py-8 md:py-12 border-b md:border-b-0 md:border-r border-white/10 flex flex-col">
                    <span class="text-white/40 text-xs font-bold uppercase tracking-[0.2em] mb-3 font-display">TIME</span>
                    <span class="text-white text-2xl font-black uppercase tracking-tight">{{formatTime event.date}}</span>
                </div>
                <!-- Location -->
                <div class="px-6 py-8 md:py-12 flex flex-col relative group">
                    <span class="text-white/40 text-xs font-bold uppercase tracking-[0.2em] mb-3 font-display">LOCATION</span>
                    <span class="text-white text-2xl font-black uppercase tracking-tight truncate">{{event.venue.name}}</span>
                    {{#if event.venue.slug}}
                    <a href="/venue/{{event.venue.slug}}" class="absolute inset-0 z-10" aria-label="View Venue"></a>
                    <i class="fas fa-arrow-right absolute right-6 top-1/2 -translate-y-1/2 text-white/20 group-hover:text-white transition-colors"></i>
                    {{/if}}
                </div>
            </div>

            <!-- Solid Geometric CTAs -->
            <div class="w-full grid grid-cols-1 sm:grid-cols-2 bg-[#050505]">
                <button id="share-button" class="w-full py-10 md:py-12 bg-[var(--color-toxic)] text-black font-black text-3xl md:text-5xl font-display uppercase tracking-tighter hover:bg-white transition-colors group">
                    SHARE
                </button>
                
                {{#if event.details.link}}
                <a href="{{event.details.link}}" target="_blank" rel="noopener noreferrer" class="w-full py-10 md:py-12 bg-[var(--color-pink)] text-black font-black text-3xl md:text-5xl font-display uppercase tracking-tighter hover:bg-white transition-colors flex items-center justify-center sm:border-l border-black/20">
                    TICKETS
                </a>
                {{/if}}
            </div>
