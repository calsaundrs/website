...
// Helper functions (getBritishTime, etc.) are included but not shown here for brevity.
// They should remain at the top of your file.
...
exports.handler = async function (event, context) {
    ...
    try {
        // --- This entire event fetching logic remains the same ---
...
<body class="antialiased"><div id="header-placeholder"><\/div><main class="container mx-auto px-8 py-16"><div class="grid lg:grid-cols-3 gap-16"><div class="lg:col-span-2"><div class="hero-image-container mb-8"><img src="${imageUrl}" alt="${eventName}" class="w-full h-full object-cover rounded-2xl"><\/div><p class="font-semibold accent-color mb-2">EVENT DETAILS<\/p><h1 class="font-anton text-6xl lg:text-8xl heading-gradient leading-none mb-4">${eventName}<\/h1><div class="mb-8">${tagsHtml}</div><div class="prose prose-invert prose-lg max-w-none text-gray-300">${description.replace(/\n/g, '<br>')}</div>${suggestedEventsHtml}<\/div><div class="lg:col-span-1"><div class="card-bg p-8 sticky top-8 space-y-6"><div><h3 class="font-bold text-lg accent-color-secondary mb-2">Date & Time<\/h3><p class="text-2xl font-semibold">${eventDate.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' })}<\/p><p class="text-xl text-gray-400">${eventDate.toLocaleTimeString('en-GB', { hour: 'numeric', minute: '2-digit', hour12: true, timeZone: 'Europe/London' })}<\/p>${recurringInfo ? `<p class="mt-2 inline-block bg-teal-400/10 text-teal-300 text-xs font-semibold px-2 py-1 rounded-full">${recurringInfo}<\/p>` : ''}<\/div><div><h3 class="font-bold text-lg accent-color-secondary mb-2">Location<\/h3>${venueHtml}<\/div>${fields['Link'] ? `<a href="${fields['Link']}" target="_blank" rel="noopener noreferrer" class="block w-full text-center bg-accent-color text-white font-bold py-4 px-6 rounded-lg hover:opacity-90 transition-opacity text-xl">GET TICKETS<\/a>` : ''}
        ...
        <div class="border-t border-gray-700 pt-6">
            ...
            <div id="share-container" class="grid grid-cols-2 gap-2">
                </div>
            <p id="copy-success" class="text-center text-green-400 text-sm mt-2" style="display: none;">Link Copied!</p>
        </div>
        
        <\/div><\/div><\/div><\/main><div id="footer-placeholder"><\/div>
        ...
        <script>
            // *** UPDATED SHARING SCRIPT (Uses Web Share API) ***
            document.addEventListener('DOMContentLoaded', () => {
                ...
                if (navigator.share) {
                    ...
                    nativeShareBtn.className = 'social-button-sm col-span-2'; // Span across both columns
                    ...
                    const fallbackHTML = \`
                        <a id="share-x" href="#" target="_blank" class="social-button-sm"><i class="fa-brands fa-x-twitter mr-2"></i>Share on X</a>
                        <a id="share-facebook" href="#" target="_blank" class="social-button-sm"><i class="fa-brands fa-facebook-f mr-2"></i>Share</a>
                        <a id="share-whatsapp" href="#" target="_blank" class="social-button-sm"><i class="fa-brands fa-whatsapp mr-2"></i>Send</a>
                        <button id="copy-link" class="social-button-sm"><i class="fa-solid fa-link mr-2"></i>Copy Link</button>
                    \`;
