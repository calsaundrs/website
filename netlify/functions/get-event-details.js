    ...
    <main class="w-full bg-[#050505] flex flex-col lg:flex-row lg:h-[calc(100vh-85px)] lg:overflow-hidden border-t border-white/10">
        ...
        <div class="w-full lg:w-1/2 flex flex-col relative bg-[#050505] h-full overflow-y-auto">
            ...
            <div class="flex-grow flex flex-col w-full">
                ...
                <nav class="w-full px-8 py-8 flex items-center justify-between text-xs font-semibold uppercase tracking-[0.2em] text-white/40 border-b border-white/5">
                ...
                <!-- Massive Title Block -->
                <div class="w-full px-8 py-10 xl:py-16 border-b border-white/5 flex-grow flex flex-col justify-center">
                    <h1 class="text-white/95 font-extrabold text-4xl sm:text-5xl md:text-6xl xl:text-7xl uppercase leading-[0.9] tracking-tight m-0 font-display break-words">
                        {{event.name}}
                    </h1>
                    
                    <!-- Auxiliary Tags -->
                    <div class="flex flex-wrap gap-2 lg:gap-3 mt-8 xl:mt-12 opacity-90">
                        {{#each event.category}}
                        <span class="text-white/70 border border-white/10 px-3 py-1 font-bold text-[10px] xl:text-xs uppercase tracking-[0.2em]">{{this}}</span>
                        {{/each}}
                        {{#if event.ageRestriction}}
                        <span class="text-[var(--color-toxic)] border border-[var(--color-toxic)]/50 px-3 py-1 font-bold text-[10px] xl:text-xs uppercase tracking-[0.2em]">{{event.ageRestriction}}</span>
                        {{/if}}
                    </div>
                </div>

                <!-- Architectural Metadata Table -->
                <div class="w-full grid grid-cols-1 sm:grid-cols-3 border-b border-white/5 bg-[#050505]">
                    <!-- Date -->
                    <div class="px-8 py-8 md:py-10 border-b sm:border-b-0 sm:border-r border-white/5 flex flex-col justify-center">
                        <span class="text-white/30 text-[10px] xl:text-xs font-bold uppercase tracking-[0.2em] mb-3 font-display">DATE //</span>
                        <span class="text-white/90 text-base xl:text-lg font-bold uppercase tracking-tight">{{formatDateOnly event.date}}</span>
                    </div>
                    <!-- Time -->
                    <div class="px-8 py-8 md:py-10 border-b sm:border-b-0 sm:border-r border-white/5 flex flex-col justify-center">
                        <span class="text-white/30 text-[10px] xl:text-xs font-bold uppercase tracking-[0.2em] mb-3 font-display">TIME //</span>
                        <span class="text-white/90 text-base xl:text-lg font-bold uppercase tracking-tight">{{formatTime event.date}}</span>
                    </div>
                    <!-- Location -->
                    <div class="px-8 py-8 md:py-10 flex flex-col justify-center relative group">
                        <span class="text-white/30 text-[10px] xl:text-xs font-bold uppercase tracking-[0.2em] mb-3 font-display">LOCATION //</span>
                        <span class="text-white/90 text-base xl:text-lg font-bold uppercase tracking-tight truncate pr-6">{{event.venue.name}}</span>
                        {{#if event.venue.slug}}
                        <a href="/venue/{{event.venue.slug}}" class="absolute inset-0 z-10" aria-label="View Venue"></a>
                        <i class="fas fa-arrow-right absolute right-8 top-1/2 -translate-y-1/2 text-white/10 group-hover:text-white/50 transition-colors"></i>
                        {{/if}}
                    </div>
                </div>
                ...
                <div class="w-full px-8 py-10 lg:py-14 border-b border-white/5 bg-[#050505]">
                    <h2 class="text-white/30 text-[10px] xl:text-xs font-bold uppercase tracking-[0.2em] mb-6 font-display">DESCRIPTION //</h2>
                    <div class="text-white/70 leading-relaxed text-sm md:text-base font-medium max-w-prose">
                ...
                <div class="w-full grid grid-cols-1 lg:grid-cols-2 border-b border-white/5 bg-[#050505]">
                    <!-- Venue Details -->
                    <div class="px-8 py-8 md:py-12 border-b lg:border-b-0 lg:border-r border-white/5 flex flex-col">
                        <h3 class="text-white/30 text-[10px] xl:text-xs font-bold uppercase tracking-[0.2em] mb-4 font-display">VENUE DETAILS //</h3>
                        <p class="text-[var(--color-toxic)] font-black text-lg mb-2 font-display uppercase">{{event.venue.name}}</p>
                        {{#if event.venue.address}}
                        <p class="text-white/50 text-sm mb-6">{{event.venue.address}}</p>
                        {{/if}}
                        {{#if event.venue.slug}}
                        <a href="/venue/{{event.venue.slug}}" class="text-white/80 text-xs font-bold uppercase tracking-[0.1em] hover:text-[var(--color-toxic)] transition-colors mt-auto w-max group"><span class="border-b border-white/20 group-hover:border-[var(--color-toxic)] pb-1">View Venue Profile</span> <i class="fas fa-arrow-right ml-1 text-white/30 group-hover:text-[var(--color-toxic)]"></i></a>
                        {{/if}}
                    </div>
                    ...
                    <div class="px-8 py-8 md:py-12 flex flex-col gap-8">
                        <div>
                            <h3 class="text-white/30 text-[10px] xl:text-xs font-bold uppercase tracking-[0.2em] mb-4 font-display">CALENDAR //</h3>
                            <div class="flex flex-col gap-4">
                                {{#if event.googleCalendarUrl}}
                                <a href="{{event.googleCalendarUrl}}" target="_blank" rel="noopener noreferrer" class="text-white/80 text-xs font-bold uppercase tracking-[0.1em] hover:text-[var(--color-toxic)] transition-colors flex items-center"><i class="fab fa-google mr-4 text-white/30"></i> Add to Google Calendar</a>
                                {{/if}}
                                {{#if event.icalUrl}}
                                <a href="{{event.icalUrl}}" download="{{event.slug}}.ics" class="text-white/80 text-xs font-bold uppercase tracking-[0.1em] hover:text-[var(--color-pink)] transition-colors flex items-center"><i class="fas fa-calendar-alt mr-4 text-white/30"></i> Download iCal / Outlook</a>
                                {{/if}}
                            </div>
                        </div>
                        ...
                        {{#if event.organizer}}
                        <div>
                            <h3 class="text-white/30 text-[10px] xl:text-xs font-bold uppercase tracking-[0.2em] mb-2 font-display">CURATED BY //</h3>
                            <p class="text-white/80 text-sm font-medium">{{event.organizer}}</p>
                        </div>
                        {{/if}}

                        {{#if event.accessibility}}
                        <div>
                            <h3 class="text-white/30 text-[10px] xl:text-xs font-bold uppercase tracking-[0.2em] mb-2 font-display">ACCESSIBILITY //</h3>
                            <p class="text-white/80 text-sm font-medium">{{event.accessibility}}</p>
                        </div>
                        {{/if}}
                        
                        {{#if event.isRecurring}}
                        <div>
                            <h3 class="text-white/30 text-[10px] xl:text-xs font-bold uppercase tracking-[0.2em] mb-2 font-display">RECURRING //</h3>
                ...
                <div class="w-full px-8 py-10 lg:py-12 bg-[#050505]">
                    <h3 class="text-white/30 text-[10px] xl:text-xs font-bold uppercase tracking-[0.2em] mb-6 font-display">OTHER DATES //</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {{#each otherInstances}}
                        <a href="/event/{{this.slug}}" class="flex items-center border border-white/5 hover:border-white/20 transition-colors bg-[#080808] group rounded">
                            <div class="bg-white/5 text-white/90 px-4 py-3 border-r border-white/5 font-bold font-display text-lg group-hover:bg-[var(--color-toxic)] group-hover:text-black transition-colors">{{this.dayOfMonth}}</div>
                            <div class="px-4 py-2 flex flex-col">
                                <span class="text-xs font-bold uppercase tracking-[0.1em] text-white/70">{{this.monthShort}} {{this.year}}</span>
                                <span class="text-[10px] text-white/30 uppercase tracking-widest">{{this.time}}</span>
                            </div>
                        </a>
                        {{/each}}
                    </div>
            ...
            <!-- Elegant Action Layout (Sticks to bottom of terminal) -->
            <div class="w-full flex flex-col sm:flex-row justify-end flex-shrink-0 p-6 lg:p-8 gap-4 border-t border-white/5 bg-[#030303]">
                <button id="share-button" class="py-4 px-8 border border-white/20 text-white font-bold text-sm tracking-[0.15em] uppercase hover:bg-white/10 transition-all rounded-full flex items-center justify-center min-w-[140px] group">
                    <i class="fas fa-share-alt mr-3 text-white/40 group-hover:text-white transition-colors"></i> SHARE
                </button>
                
                {{#if event.details.link}}
                <a href="{{event.details.link}}" target="_blank" rel="noopener noreferrer" class="py-4 px-10 bg-[var(--color-pink)] text-black font-extrabold text-sm tracking-[0.15em] uppercase hover:bg-white transition-all rounded-full flex items-center justify-center min-w-[200px] shadow-[0_0_15px_rgba(255,20,147,0.3)] hover:shadow-[0_0_25px_rgba(255,255,255,0.4)]">
                    GET TICKETS
                </a>
                {{/if}}
            </div>
